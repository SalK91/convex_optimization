# Model-Free Policy Evaluation: Learning the Value of a Fixed Policy

The foundation of Reinforcement Learning begins with Dynamic Programming (Policy and Value Iteration), which assumes the agent has a perfect model of the environment (the transition probabilities $P$ and the reward function $R$). In reality, however, the model is almost always unknown. This necessitates a shift to Model-Free Reinforcement Learning, where the agent must learn the values of states and actions solely from direct experience (i.e., collecting trajectories of states, actions, and rewards).

This article focuses on Model-Free Policy Evaluation, the process of estimating the State Value Function $V^\pi(s)$ for a fixed policy $\pi$ without access to $P$ or $R$.


## Learning from Direct Experience

The agent's data is generated by executing the fixed policy $\pi$ in the environment, yielding episodes (or trajectories) of experience:
$$
s_0, a_0, r_1, s_1, a_1, r_2, s_2, \ldots, s_T
$$
Our goal is to estimate $V^\pi(s)$, which by definition is the expected return ($\mathbb{E}[G_t]$) starting from state $s$.

### The Challenge of Unknown Models
In the Model-Free setting, we cannot use the Bellman Expectation Equation directly because it requires knowing the transition probability $P(s'|s, a)$:
$$
V^\pi(s) = \sum_a \pi(a \mid s) \left[ R(s,a) + \gamma \sum_{s'} P(s'|s,a) V^\pi(s') \right]
$$
Instead, we must approximate the expectation using averages over the collected samples.

 
## Monte Carlo (MC) Policy Evaluation

Monte Carlo (MC) methods are the simplest approach to model-free evaluation. The core idea is that since the true value function $V^\pi(s)$ is the expected return, we can approximate it by simply averaging the observed returns ($G_t$) from many episodes that start at state $s$.
...
This example demonstrates how First-Visit Monte Carlo (MC) and Every-Visit Monte Carlo (MC) calculate value estimates after three complete episodes, highlighting how the two methods handle revisiting the same state within an episode.


## Temporal Difference (TD) Learning

While MC uses the full return $G_t$, Temporal Difference (TD) learning is the fundamental shift in policy evaluation. It retains the concept of the incremental update but changes the target, introducing a technique called bootstrapping.
