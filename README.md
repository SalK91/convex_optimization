...
Trust refion algotyjms
what is conjugate gradient method
...
Conjugate Gradient (CG)	Ch. 12 (after Newton)	Core method for large-scale QPs and smooth convex problems.
Nelder–Mead, Simulated Annealing	Ch. 15	Needed for derivative-free optimization overview.
Evolutionary / Genetic Algorithms	Ch. 15	Bridges convex and nonconvex stochastic optimization.
Mirror Descent / Online Convex Optimization	Ch. 15	Central for ML (AdaGrad, FTRL, regret bounds).
Proximal point method and Moreau envelope	Ch. 14	Gives theoretical backbone for proximal algorithms.
Augmented Lagrangian (ALM) and ADMM derivation	Ch. 14	Fundamental for distributed ML.
Dual averaging and stochastic mirror descent	Ch. 15	Links convex optimization to adaptive gradient methods (Adam, RMSProp).
